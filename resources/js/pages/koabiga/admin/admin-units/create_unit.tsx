...
export default function CreateUnit({ availableLeaders = [], zones = [] }: CreateUnitProps) {
    ...
    const generateUnitCode = async (zoneId: string) => {
        ...
        try {
            ...
            const response = await fetch('/koabiga/admin/admin-units/generate-code', {
                method: 'POST',
                headers: {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await router.post('/koabiga/admin/admin-units', {
                ...formData,
                leader_id: formData.leader_id === 'none' ? null : formData.leader_id
            }, {
                onSuccess: () => {
                    router.visit('/koabiga/admin/admin-units');
                },
                onError: (errors) => {
                    setErrors(errors);
    ...
    return (
        <AppLayout breadcrumbs={[
            ...
            { title: 'Create Unit', href: '/koabiga/admin/admin-units/create' }
        ]}>
